    ...
    public Script()
    {
        ...
        public void Execute(ScriptContext context /*, System.Windows.Window window*/)
        {
            ...
            Patient patient = context.Patient;
                Structure PTV = plan.StructureSet.Structures.Where(s => s.Id.ToLower().Contains("ptv_01")).First();
                ...
                Structure esophagus = plan.StructureSet.Structures.Where(s => s.Id.ToLower().Contains("esophagus")).Single();
            ...
            DoseValue cordDG = new DoseValue(2250, DoseValue.DoseUnit.cGy);
                ...
                DoseValue esophaDG = new DoseValue(1230, DoseValue.DoseUnit.cGy);
                DoseValue esophaPG = new DoseValue(1800, DoseValue.DoseUnit.cGy);
                DoseValue aortaDG = new DoseValue(4700, DoseValue.DoseUnit.cGy);
                DoseValue brachiDG = new DoseValue(2040, DoseValue.DoseUnit.cGy);
                DoseValue brachiPG = new DoseValue(2400, DoseValue.DoseUnit.cGy);
                DoseValue PTVG = new DoseValue(5400, DoseValue.DoseUnit.cGy);
                DoseValue v50G = new DoseValue(2700, DoseValue.DoseUnit.cGy);
                DoseValue v105 = new DoseValue(5670, DoseValue.DoseUnit.cGy);
                DoseValue v100G = new DoseValue(5400, DoseValue.DoseUnit.cGy);
                DoseValue v20G = new DoseValue(2000, DoseValue.DoseUnit.cGy);
                DoseValue tracheaDG = new DoseValue(1500, DoseValue.DoseUnit.cGy);
                ...
                DoseValue chestVD = new DoseValue(2880, DoseValue.DoseUnit.cGy);
                DoseValue v90 = new DoseValue(4860, DoseValue.DoseUnit.cGy);


                double cordV = plan.GetVolumeAtDose(cord, cordDG, VolumePresentation.AbsoluteCm3);
                ...
                double V20_volR = plan.GetVolumeAtDose(lungTotal, v20G, VolumePresentation.Relative);
                double V20_volL = plan.GetVolumeAtDose(lungL, v20G, VolumePresentation.Relative);
                double esophagV = plan.GetVolumeAtDose(esophagus, esophaDG, VolumePresentation.AbsoluteCm3);
                double cordV2 = plan.GetVolumeAtDose(esophagus, esophaPG, VolumePresentation.AbsoluteCm3);
                ...
                double chestV = plan.GetVolumeAtDose(chestwall, chestVD, VolumePresentation.AbsoluteCm3);
            ...
            DoseValue aortaVD = plan.GetDoseAtVolume(aorta, 10, VolumePresentation.AbsoluteCm3, DoseValuePresentation.Absolute);
                ...
                DoseValue cordP = plan.GetDVHCumulativeData(cord, DoseValuePresentation.Absolute, VolumePresentation.AbsoluteCm3, 0.01).MaxDose;
                DoseValue aortaP = plan.GetDVHCumulativeData(aorta, DoseValuePresentation.Relative, VolumePresentation.AbsoluteCm3, 0.1).MaxDose;
            ...
            double hetero = 540000 / ptvmax.Dose;
            ...
            finalSummary += "V100 / PTV Volume:," + Math.Round(D100_vol / PTV_vol, 2) + ',' + PWF(1.4, 1.2, D100_vol / PTV_vol) + '\n';
            finalSummary += "V50 Volume:," + Math.Round(D50_vol, 2) + " cc\n";
            finalSummary += "V50_volume / PTV volume:," + Math.Round(D50_vol / PTV_vol, 2) + ',' + PWF(-65E-4 * PTV_vol + 4.1368, -65E-4 * PTV_vol + 3.9368, D50_vol / PTV_vol) + '\n';

            finalSummary += "2cm max dose:," + mm2P.ToString() + ',' + PWF(-5E-3 * Math.Pow(PTV_vol, 2) + .1743 * PTV_vol + 26.007, -4E-3 * PTV_vol * PTV_vol + .1562 * PTV_vol + 24.366, mm2P.Dose) + '\n';
            finalSummary += "Critical Structures\n";
            finalSummary += "Lung V20: (percent of Volume <20%)," + Math.Round(V20_volR, 2) + '%' + ',' + PWF(15, 10, V20_volR) + '\n';
            finalSummary += "Spinal Cord Point Max < 21.9 Gy," + cordP.ToString() + ',' + PWF(2190, 2000, cordP.Dose) + '\n';
                finalSummary += "Spinal Cord V@ 18 Gy: (<.35cc)," + Math.Round(cordV, 5) + " cc," + PWF(.35,.2, cordV) + "\n";
                finalSummary += "Spinal Cord V@ 12.3 Gy: (<1.2cc)," + Math.Round(cordV2, 5) + " cc," + PWF(1.2, .8, cordV2) + "\n";
            finalSummary += "Aorta V@ 47Gy: (<10cc)," + Math.Round(aortaV, 2) + ',' + PWF(10, 8, aortaV) + '\n';
            finalSummary += "Aorta max point dose: (<105% Rx)," + aortaP.ToString() + ',' + PWF(105, 100, aortaP.Dose) + '\n';



            finalSummary += "Brachial Plexus volume @20.4Gy: (<3cc)," + Math.Round(brachiV, 2) + " cc," + PWF(3, 2.5, brachiV) + "\n";
                finalSummary += "Brachial Plexus max point dose: (<24Gy)," + brachiP.ToString() + ','+ PWF(2400, 2000, brachiP.Dose) +'\n';
            finalSummary += "Esophagus Max Point Dose: (<25.2)," + esophagP.ToString() + ',' + PWF(25.2, 20, esophagP.Dose) + '\n';
            finalSummary += "Esophagus V@ 17.5 Gy: (<5cc)," + Math.Round(esophagV, 2) + "cc," + PWF(5, 4, esophagV) + '\n';
            finalSummary += "Heart V@24 Gy: (<15cc)," + Math.Round(heartV, 2) + " cc,"+ PWF(15,10,heartV) +"\n";
                finalSummary += "Heart Point Max: (<30Gy)," + heartP.ToString() + ','+ PWF(3800, 3500, heartP.Dose) + '\n';
                
                finalSummary += "Trachea V@15 Gy: (<4cc), " + Math.Round(tracheaV, 2) + " cc,"+ PWF(4, 3, tracheaV) +"\n";
                finalSummary += "Trachea Point Max: (<30Gy)," + tracheaP.ToString() + ',' + PWF(3000, 2500, tracheaP.Dose) + '\n';
                finalSummary += "Skin V@30 Gy: (<10cc)," + Math.Round(skinV, 2) + " cc,"+ PWF(10, 8, skinV) +"\n";
                finalSummary += "Skin Point Max: (<33Gy)," + skinP.ToString() + ','+ PWF(3300,3000, skinP.Dose) +'\n';
                ...
                finalSummary += "Chest Wall V@28.8 Gy: (<1cc)," + Math.Round(chestV, 2) + " cc,"+ PWF(1, .5, chestV) +"\n";
                finalSummary += "Chest Wall Point Max: (<36.9Gy)," + chestP.ToString() + ',' +PWF(3690, 2500, chestP.Dose)+ '\n';
                ...
                StreamWriter Sw = new StreamWriter("v:\\Special_Phys_Reports\\"+ patient.LastName + '_' + patient.Id+ '_' + "Lung3x18.csv");
